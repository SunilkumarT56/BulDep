  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        'https://untolerative-len-rumblingly.ngrok-free.dev/auth/login',
        {
          method: 'POST',
          headers: {
      ...
      if (data.authenticated && data.user) {
        ...
        onLogin(data.user);
        window.location.href = '/yt-pipeline-dashboard';
        return;
      }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-black text-white p-4 font-sans relative overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-primary/20 blur-[120px] rounded-full mix-blend-screen opacity-50 animate-pulse-slow" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[500px] h-[500px] bg-blue-600/10 blur-[100px] rounded-full mix-blend-screen opacity-30" />
              ...
              <Button
                ...
                onClick={() =>
                  (window.location.href =
                    'https://untolerative-len-rumblingly.ngrok-free.dev/auth/github')
                }
              >
