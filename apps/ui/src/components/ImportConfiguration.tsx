  ...
  const handleExpand = async (e: React.MouseEvent, forceOpen?: boolean) => {
    ...
    try {
      ...
      const response = await fetch(
        'https://untolerative-len-rumblingly.ngrok-free.dev/user/new/inner-dir',
        {
          method: 'POST',
          headers: {
        ...
        {isExpandable ? (
          <span
            className={`transition-colors ${
              isSelected ? 'text-zinc-300' : 'text-zinc-600 group-hover:text-zinc-400'
            }`}
          >
            {loading ? (
...
export function ImportConfiguration({ repo, onBack, onDeploy, loading }: ImportConfigurationProps) {
  const [projectname, setprojectname] = useState(repo.projectname || repo.name);
  ...
  const handleDirectorySelect = async (dir: string) => {
    ...
    try {
      ...
      const response = await fetch(
        'https://untolerative-len-rumblingly.ngrok-free.dev/user/new/framework',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className="w-full max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl shadow-zinc-950/50">
        ...
        <div className="p-6 border-b border-zinc-800 bg-zinc-900/30">
          <h2 className="text-xs uppercase tracking-wider font-semibold text-zinc-500 mb-3">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-white text-lg">
                  {repo.owner?.login || 'User'} / {repo.name}
                </span>
                <span className="px-2 py-0.5 text-[10px] bg-zinc-800 text-zinc-400 rounded-full border border-zinc-700 font-mono">
                  Public
                </span>
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <select
                  ...
                  className={`w-full h-10 bg-zinc-950 border border-zinc-800 rounded-md px-3 text-sm text-white appearance-none focus:outline-none focus:border-white/30 transition-colors cursor-pointer ${
                    detectingFramework ? 'opacity-50 cursor-not-allowed' : ''
                  }`}
                >
                  {frameworks.map((f) => (
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <Input
                    ...
                    className={`bg-zinc-950 border-zinc-800 focus:border-white/30 transition-colors h-10 font-mono text-sm w-full cursor-pointer ${
                      detectingFramework ? 'opacity-50' : ''
                    }`}
                    placeholder="./"
                  />
                </div>
              ...
              {showDirList && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-zinc-950 border border-zinc-800 rounded-lg shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                  <div className="max-h-60 overflow-y-auto p-1">
                    <div className="px-2 py-1.5 text-[10px] uppercase font-bold text-zinc-600 tracking-wider">
                      Select Directory
                    </div>
                    {repo.directories ? (
                  ...
                  <div className="grid gap-4">
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className="bg-black border-zinc-800 h-9 text-sm font-mono text-zinc-300"
                      />
                    </div>
                    <div className="space-y-1">
