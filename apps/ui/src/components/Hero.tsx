...
export function Hero() {
    ...
    const fetchRepos = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://untolerative-len-rumblingly.ngrok-free.dev/user/new?page=${currentPage}&search=${searchQuery}`,
          {
            headers,
          },
        );
  ...
  const handleManualImport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        'https://untolerative-len-rumblingly.ngrok-free.dev/user/github-url',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleFinalDeploy = async (config: ProjectConfig) => {
    ...
    try {
      ...
      const response = await fetch(
        'https://untolerative-len-rumblingly.ngrok-free.dev/user/new/deploy',
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  return (
    <div className="min-h-screen bg-black text-white pt-28 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto flex flex-col items-start gap-12 relative">
      ...
      <div className="w-full relative z-10 space-y-2">
        {/* Removed motion */}
        <h1 className="text-4xl font-bold tracking-tight text-white mb-2">New Project</h1>
      ...
      <div className="w-full flex flex-col lg:flex-row gap-8 relative z-10 pb-20">
                  ...
                  <>
                    {repos.map((repo) => (
                      <div
                        ...
                        className={`group relative flex items-center justify-between p-4 bg-black hover:bg-zinc-900 transition-colors cursor-pointer border-b border-zinc-800 last:border-0 ${
                          importingRepoId === repo.id ? 'opacity-70' : ''
                        }`}
                      >
        ...
        <div className="w-full lg:w-2/5">
          <div
            className={`sticky top-32 rounded-xl border border-zinc-800 overflow-hidden flex flex-col min-h-[500px] transition-all duration-300 ${
              !previewData ? 'bg-black' : 'bg-black'
            }`}
          >
            {previewData ? (
                ...
                <div className="p-8 border-b border-zinc-800 bg-black">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono border border-zinc-800 bg-zinc-900 text-zinc-400 flex items-center gap-1.5">
                        <div className="w-1.5 h-1.5 rounded-full bg-emerald-500/50"></div>
                        {previewData.default_branch}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {previewData.commits && previewData.commits.length > 0 ? (
                      previewData.commits.slice(0, 5).map((commit, i) => (
                        <div
                          key={i}
                          className="flex flex-col gap-1 p-3 rounded-md border border-zinc-800 bg-zinc-900/10"
                        >
                ...
                {previewData.languages && Object.keys(previewData.languages).length > 0 && (
                  <div className="p-6 border-t border-zinc-800 bg-black">
                    <div className="flex h-2 w-full overflow-hidden rounded-full bg-zinc-900 mb-3">
                      {(() => {
                        const total = Object.values(previewData.languages).reduce(
                          (a, b) => a + b,
                          0,
                        );
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2">
                      {Object.keys(previewData.languages)
                        .slice(0, 4)
                        .map((lang, i) => (
                          <div
                            key={lang}
                            className="flex items-center gap-2 text-[10px] font-medium text-zinc-400"
                          >
                            <div
                              className={`h-2 w-2 rounded-full ${
                                ['bg-zinc-100', 'bg-zinc-300', 'bg-zinc-500', 'bg-zinc-700'][i % 4]
                              }`}
                            />
                            <span>{lang}</span>
                          </div>
              ...
              <div className="flex flex-col items-center justify-center flex-1 h-full text-zinc-600 gap-6 p-10 text-center">
                ...
                <div className="max-w-[240px] space-y-1">
                  ...
                  <p className="text-xs text-zinc-500">Select a repository to see details.</p>
                </div>
